...
final private[worker] class StorageManager(conf: CelebornConf, workerSource: AbstractSource)
  ...
  def createDiskFile(
    ...
    while (retryCount < conf.workerCreateWriterMaxAttempts) {
      ...
      if ((dirs.isEmpty || hasHDFSStorage) && location.getStorageInfo.HDFSAvailable()) {
        val shuffleDir =
      ...
      } else if ((dirs.isEmpty || hasS3Storage) && location.getStorageInfo.S3Available()) {
        val shuffleDir =
      ...
      } else if ((dirs.isEmpty || hasOssStorage) && location.getStorageInfo.OSSAvailable()) {
        val shuffleDir =
